...
class Espncricinfo {
	...
	private Batsman[] batsman = new Batsman[SIZE];
	...
	public int addBatsman(String name, int runsScored, int centuries, int halfCenturies, int ballsFaced, int fours, int sixes){
		if(noOfBatsmen < SIZE-1){
			Batsman newBatsman = new Batsman(name, runsScored, centuries, halfCenturies, ballsFaced, fours, sixes);
			batsman[noOfBatsmen] = newBatsman;
			noOfBatsmen+=1;
			return newBatsman.getId();
		}
		return 0;
	}
	...
	public Batsman updateBatsmanStats(int id, int runsScored, int fours, int sixes, int ballsFaced){
		for(int i=0; i < SIZE; i++){
			if(batsman[i].getId() == id){
				batsman[i].setRunsScored(runsScored);
				batsman[i].setFours(fours);
				batsman[i].setSixes(sixes);
				batsman[i].setBallsFaced(ballsFaced);
				return batsman[i];
			}
		}
		return null;
